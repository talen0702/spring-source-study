...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				//注册拦截Bean创建的Bean理器 这里只是注册，真正的调用是在getBean时候
				...
				//完成刷新过程，通知生命周期处现器lifecycleProcessor刷新过程，同时发出ContextRefreshedEvent通知别人
				finishRefresh();
			}
	...
	protected void prepareRefresh() {
		...
		//自己写一个类重写了initPropertySources方法，在该方法中设置了一个环境变量的值为XXX启动的时候，我的环境变量中没有该值就会启动抛出异常
		//留给子类处理
	...
	/**
	 * 方法实现说明:获取我们spring上下文对象的父上下文，
	 * 然后通过父上下文获取父工厂Spring根的上下文对象不会有父容器，但是我们的springmvc就在这里存在父工厂.
