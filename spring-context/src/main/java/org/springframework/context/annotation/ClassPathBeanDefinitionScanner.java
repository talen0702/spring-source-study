	...
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		...
		for (String basePackage : basePackages) {
			...
			for (BeanDefinition candidate : candidates) {
				//获得bean定义类中@Scope注解的值，bean的作用域
				ScopeMetadata scopeMetadata = this.scopeMetadataResolver.resolveScopeMetadata(candidate);
				//设置作用域
				candidate.setScope(scopeMetadata.getScopeName());
				//为bean生成名字
				String beanName = this.beanNameGenerator.generateBeanName(candidate, this.registry);
				//如果扫描到的bean不是spring的注解bean,则为bean设置默认值，BeanDefinitionDefaults
				//设置bean的自动依赖注入装配属性
				//autowireMode(自动装配模型:AutowireCapableBeanFactory.AUTOWIRE_NO/AUTOWIRE_BY_NAME/AUTOWIRE_BY_TYPE/AUTOWIRE_CONSTRUCTOR)，
				//dependencyCheck(AutowireCapableBeanFactory.DEPENDENCY_CHECK_NONE,DEPENDENCY_CHECK_OBJECTS,DEPENDENCY_CHECK_SIMPLE,DEPENDENCY_CHECK_ALL)，
				//initMethodName，destroyMethodName，enforceDestroyMethod=false,enforceInitMethod=false
